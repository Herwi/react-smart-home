...
type DispatchFunction = (
  action: IAction<ISmartDevice[] | ISmartDeviceDetails | string | void>
) => void;
...
export const loadDevicesAsync = () => (dispatch: DispatchFunction) => {
  ...
  DeviceService.getAllDevices()
    .then((response) =>
      dispatch(actions.devicesLoadSuccess(response.data as ISmartDevice[]))
    )
    .catch((error) => dispatch(actions.devicesLoadError(error.message)));
};

export const loadDeviceDetailsAsync =
  (id: string) => (dispatch: DispatchFunction) => {
    ...
    DeviceService.getDeviceDetails(id)
      ...
      .catch((error) =>
        dispatch(actions.deviceDetailsLoadError(error.message))
      );
  };
